...
// printCommitsStats prints the commit stats
...
// printMonth prints the month names in the first line, determining when the month
// changed between switching weeks
func printMonth() {
	week := getBeginningOfDay(time.Now()).Add(-(daysInLastSixMonth * time.Hour * 24))
	month := week.Month()
	fmt.Printf("         ")
	for {
		if week.Month() != month {
